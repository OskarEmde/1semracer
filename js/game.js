...
async function setupAudioSystem() {
    ...
    await state.audio.loadSounds();
    state.audio.attachMotorKeys();
}
async function playIntro() {
    ...
    // Vent på at brugeren klikker "Start"
    await new Promise(resolve => {
        startBtn.addEventListener("click", resolve, { once: true });
    });

    // Skjul knapperne og start video + lyd
    if (buttons) buttons.style.display = "none";
    if (video) {
        video.style.display = "block";
        video.play();
    }
    state.audio.playSound("intro");

    // Vent til videoen er færdig
    if (video) {
        await new Promise(resolve => {
    ...
    // Fade overlay væk og start musik
    overlay.style.transition = "opacity 1s";
    overlay.style.opacity = "0";
    await new Promise(resolve => setTimeout(resolve, 1000));
    state.audio.playSound("music", true);
    overlay.remove();
}
